...
const ExpandableButton: FC<{
    ...
    onToggle: () => void;
    text: string;
}> = ({expanded, insideProgramPage, minWidth, onToggle, text}) => (
...
export const TopStickyRow: FC<{
    ...
    const expandableButton = (minWidth: string, text: string) => (
        <ExpandableButton
            expanded={expanded}
    ...
    return (
        ...
        >
            <div>
                <Cell item={expandableButton(columnWidthMap[0], "申请人")} width={columnWidthMap[0]}
                      style={{marginRight: insideProgramPage ? '5px' : '0px'}}
                />
                {insideProgramPage || <Cell item={expandableButton(columnWidthMap[1], "申请项目")} width={columnWidthMap[1]}/>}
                <Cell item={expandableButton(columnWidthMap[2], "申请结果")} width={columnWidthMap[2]}
                      style={{marginRight: insideProgramPage ? '15px' : '0px'}}
                />
                <Cell item={expandableButton(columnWidthMap[3], "最终去向")} width={columnWidthMap[3]}
                      style={{marginRight: insideProgramPage ? '15px' : '0px'}}
                />
                <Cell item={expandableButton(columnWidthMap[4], "申请季")} width={columnWidthMap[4]}
            ...
            <div style={{verticalAlign: "middle", paddingLeft: "10px"}}>
                <InlineTypography component="span" sx={{gap: "0.5rem"}}>
                    ...
                    <Tooltip title="添加申请记录" arrow>
                        ...
                        >
                            <ControlPointIcon sx={{fontSize: "0.9rem"}}/>
                        </IconButton>
                    </Tooltip>
                </InlineTypography>
            </div>
...
const timelineBodyTemplate = (rowData: RecordData, timelineKey: keyof RecordData["TimeLine"]) => (
...
export const PlainTable: FC<{
    ...
    const groupFirstRecord: RecordData[] = [];
