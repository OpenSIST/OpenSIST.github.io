...
export function GuidanceGrid1({Index, TitlePrime, TitleSecond, Content, ButtonLink}) {
...
export function GuidanceGrid2({Index, TitlePrime, TitleSecond, Content, ButtonLink}) {
    const nameList = ['profile', 'datapoints', 'programs', 'posts'];
    const theme = useTheme();
    return (
        <Card sx={{
            ...
            < CardActionArea component={Link} to={ButtonLink} sx={{display: 'flex', justifyContent: 'start', flexDirection: (Index % 2 ? 'row' : 'row-reverse')}}>
                <CardMedia
                    component="img"
                    sx={{
                        width: {
